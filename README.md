# Forestry & Trees 2024 🌳  
מערכת BI לניתוח דוחות כריתה וערעורים – "רחובות של עצים"

ברוכים הבאים למערכת **Forestry & Trees 2024** –  
ממשק BI אינטראקטיבי לניתוח דוחות כריתה ודוחות ערעורים של פרויקט "רחובות של עצים", בצורה נוחה, ויזואלית ומוכנה למצגות.

המערכת מאפשרת:

- 🧩 **מיזוג וניקוי נתונים** מדוחות כריתה גולמיים.
- 📊 **ניתוח BI לדוחות כריתה** – לפי יישובים, מיני עץ, שנים, אזורים וסיבות.
- 📝 **ניתוח BI לדוחות ערעורים** – אחוזי הצלחה, עצים שניצלו לשימור, יישובים מובילים ועוד.

---

## 📚 תוכן עניינים

1. [ת overview קצר – מה המערכת עושה?](#-ת-overview-קצר--מה-המערכת-עושה)
2. [מסלול עבודה מומלץ](#-מסלול-עבודה-מומלץ)
3. [מודולים מרכזיים במערכת](#-מודולים-מרכזיים-במערכת)
4. [אילו קבצים נדרשים?](#-אילו-קבצים-נדרשים)
5. [איך להכין את קבצי האקסל (שלב-אחר-שלב)](#-איך-להכין-את-קבצי-האקסל-שלב-אחר-שלב)
6. [הרצת המיזוג – דף 🧩 Merge & Export](#-הרצת-המיזוג--דף--merge--export)
7. [עבודה עם קובץ המיזוג – דף 📊 BI דוחות כריתה](#-עבודה-עם-קובץ-המיזוג--דף--bi-דוחות-כריתה)
8. [ניתוח דוחות ערעורים – דף 📝 BI ערעורים](#-ניתוח-דוחות-ערעורים--דף--bi-ערעורים)
9. [מסלול עבודה מומלץ – סיכום](#-מסלול-עבודה-מומלץ--סיכום)
10. [התקנה והרצה טכנית (Python + Streamlit)](#-התקנה-והרצה-טכנית-python--streamlit)
11. [הפרויקט והשותפים](#-הפרויקט-והשותפים)
12. [שיפור עתידי והערות](#-שיפור-עתידי-והערות)

---

## 🔎 ת overview קצר – מה המערכת עושה?

**מערכת Forestry & Trees 2024** מיועדת לשלושה שימושים עיקריים:

1. **מיזוג וניקוי נתונים**  
   - איחוד כל דוחות הכריתה לקובץ אחד.
   - פיענוח קודים (יישוב, מין עץ, פעולה, סיבה).
   - תיקון תאריכים וניקוי רשומות בעייתיות.

2. **יצירת קובץ BI מאוחד לכריתה**  
   - הפקת קובץ מרכזי:  
     `merged_forest_reports_FINAL_dates_fixed.xlsx`  
   - קובץ זה מהווה בסיס מסודר ל:
     - Streamlit
     - Power BI
     - Excel Pivot
     - וכל כלי BI נוסף.

3. **ניתוח BI לדוחות כריתה ודוחות ערעורים**  
   - דף BI לכריתה: TOP-יישובים, TOP-עצים, פילוחים לפי זמן/אזור/סיבה.
   - דף BI לערעורים: אחוזי הצלחה, עצים שניצלו לשימור, יישובים מובילים ועוד.

---

## 🧭 מסלול עבודה מומלץ

מסלול זרימה מומלץ במערכת:

> **🧩 Merge & Export → 📊 BI כריתה → 📝 BI ערעורים**

1. **Merge & Export – מיזוג ופיענוח**
   - מיזוג כל גליונות הכריתה לקובץ אחד.
   - המרת קודים (יישוב / עץ).
   - פיענוח פעולה/סיבה.
   - תיקון תאריכים.
   - תוצאה: קובץ  
     `merged_forest_reports_FINAL_dates_fixed.xlsx` – מוכן ל־BI.

2. **BI – דוחות כריתה**
   - ניתוח ויזואלי של כריתות:
     - TOP-יישובים.
     - TOP-מיני עץ.
     - פילוח לפי שנים, אזורים, סיבות כריתה.
   - גרפים אינטראקטיביים + אפשרות ייצוא ל־PNG / CSV.

3. **BI – דוחות ערעורים**
   - ניתוח ערעורים:
     - אחוזי הצלחה.
     - עצים שניצלו לשימור.
     - יישובים מובילים.
     - סיבות ערעור וסטטוס החלטה.

4. **📘 מדריך שימוש**
   - דף ייעודי עם הסבר מפורט צעד־אחר־צעד:
     - אילו קבצים צריך.
     - איך להכין אותם באקסל.
     - איך לעבוד בכל דף במערכת.

---

## 🧩 מודולים מרכזיים במערכת

- **🧩 Merge & Export – מיזוג ופיענוח**
  - מיזוג כל גליונות הכריתה לקובץ אחד.
  - מיפוי עמודות לשמות אחידים.
  - פיענוח קודי יישוב / עץ.
  - תיקון תאריכים ויצירת דגלים (`__is_cut__`, `__is_move__`).

- **📊 BI – דוחות כריתה**
  - ניתוח נתוני כריתה:
    - TOP-10 יישובים בכמות כריתות.
    - TOP-10 מיני עץ שנכרתו.
    - פילוח לפי סיבות, אזורים, טווחי תאריכים ועוד.

- **📝 BI – דוחות ערעורים**
  - ניתוח ערעורים:
    - אחוזי הצלחה.
    - יישובים עם הכי הרבה ערעורים.
    - עצים שניצלו לשימור.
    - סטטוס ההחלטות.

- **📘 מדריך שימוש**
  - הסבר מלא על הכנת קבצים, הרצת המיזוג והעבודה עם דפי ה-BI.

---

## 📂 אילו קבצים נדרשים?

כדי לעבוד עם המערכת באופן מלא, נדרשים:

### 2.1 דוחות כריתה מאוחדים

קובץ מקורי מהגוף הממשלתי, לדוגמה:  
`forestry_and_trees_report2024 דוחות כריתה אחוד.xlsx`

שכולל:

- מספר גליונות כריתה לפי אזורים/מחוזות.
- גיליון **"רשימת ערים לפי קודים"**.
- גיליון **"רשימת עצים לפי קודים"**.

### 2.2 קובצי רשימות ייעודיים (שוטף מהמחשב שלך)

שלושה קבצים נפרדים:

1. **קובץ 1 – דוחות כריתה מאוחדים (ללא גליונות רשימות)**  
   - הקובץ המקורי, לאחר הסרת גליונות הרשימות.

2. **קובץ 2 – `רשימת ערים לפי קודים.xlsx`**  
   - כולל עמודות כמו:  
     `ישוב`, `סמל ישוב`.

3. **קובץ 3 – `רשימת עצים לפי קודים.xlsx`**  
   - כולל עמודות כמו:  
     `Tree` (קוד העץ), `שם עץ` (שם העץ בעברית).

### 2.3 קובץ דוחות ערעורים

קובץ נפרד, לדוגמה:  
`forestry_and_trees_ararim2024- דוח ערערים.xlsx`  
משמש רק לדף BI של הערעורים.

> 💡 **טיפ ארגוני**:  
> מומלץ ליצור תיקייה מסודרת, למשל:  
> `TreeProjectsBI/inputs`  
> ולשמור שם את 4 הקבצים:
> - דוחות כריתה מאוחדים (ללא רשימות)
> - רשימת ערים
> - רשימת עצים
> - דוח ערעורים

---

## 🧾 איך להכין את קבצי האקסל (שלב-אחר-שלב)?

### 3.1 פתיחת הקובץ המקורי

1. פתח/י באקסל את הקובץ המקורי, לדוגמה:  
   `forestry_and_trees_report2024 דוחות כריתה אחוד.xlsx`
2. ודא/י שיש בו גליונות:
   - **"רשימת ערים לפי קודים"**
   - **"רשימת עצים לפי קודים"**

### 3.2 שמירת רשימת ערים לקובץ נפרד

1. עמוד/י על הגיליון: **"רשימת ערים לפי קודים"**.
2. File → Save As → שמירת הקובץ בשם:  
   `רשימת ערים לפי קודים.xlsx`
3. ודא/י שהעמודות המרכזיות:
   - `ישוב`
   - `סמל ישוב`

### 3.3 שמירת רשימת עצים לקובץ נפרד

1. חזור/י לקובץ המקורי.
2. עבור/י לגיליון: **"רשימת עצים לפי קודים"**.
3. שמור/י כקובץ חדש:  
   `רשימת עצים לפי קודים.xlsx`
4. ודא/י שיש בו עמודות:
   - `Tree` – קוד העץ.
   - `שם עץ` – שם העץ בעברית.

### 3.4 יצירת קובץ דוחות כריתה "ללא רשימות"

1. בקובץ המקורי (הכולל גם רשימות):
   - מחק/י או הסתר/י את הגיליונות:
     - **"רשימת ערים לפי קודים"**
     - **"רשימת עצים לפי קודים"**
   - שמור/י כעותק חדש בשם לדוגמה:  
     `forestry_and_trees_report2024 דוחות כריתה אחוד ללא הרשימות.xlsx`
2. זה יהיה **קובץ הדוחות הראשי** שתעלה/י בדף **🧩 Merge & Export**.

---

## 🧩 הרצת המיזוג – דף 🧩 Merge & Export

ב־Streamlit, בסרגל הצד, בחר/י:

> **🧩 Merge & Export**

### 4.1 העלאת הקבצים

במסך המיזוג תופיע טופסת העלאה עם שלושה שדות:

1. קובץ דוחות כריתה מאוחד ללא גליונות רשימות  
   `forestry_and_trees_report2024 דוחות כריתה אחוד ללא הרשימות.xlsx`
2. `רשימת ערים לפי קודים.xlsx`
3. `רשימת עצים לפי קודים.xlsx`

לאחר בחירת שלושת הקבצים לחץ/י על:

> **🚀 "הרץ מיזוג והמרות"**

### 4.2 מה עושה המנוע בזמן הריצה?

- קריאה של **כל גליונות הכריתה** בקובץ הראשי.
- איתור אוטומטי של שורת הכותרת גם אם:
  - שורה 1 היא כותרת כללית.
  - שורה 2 ריקה.
  - הכותרות האמיתיות בשורה 3/4.
- מיפוי עמודות לשמות אחידים:
  - אזור, יישוב, שם מין עץ, תאריכים (`מ-תאריך`, `עד-תאריך`), פעולה, סיבה ועוד.
- המרת:
  - **קודי יישוב → שם יישוב** (בעזרת קובץ רשימת ערים).
  - **קודי עץ → שם מין עץ** (בעזרת קובץ רשימת עצים).
- פיענוח:
  - `פעולה` → `פעולה_מפוענחת` (כריתה / העתקה).
  - `סיבה` → `סיבה_מפוענחת` (בטיחות, בנייה, עץ מת, וכו').
- יצירת דגלים:
  - `__is_cut__` – האם השורה היא כריתה.
  - `__is_move__` – האם מדובר בהעתקה/שימור.

### 4.3 תוצאה – קובץ BI מוכן

בסוף התהליך יופיע כפתור הורדה לקובץ:

> `merged_forest_reports_FINAL_dates_fixed.xlsx`

זהו **הקובץ המרכזי** שאיתו עובדים בדף **📊 BI – דוחות כריתה**,  
ואפשר להשתמש בו גם ב־Power BI, Pivot Table ועוד.

---

## 📊 עבודה עם קובץ המיזוג – דף 📊 BI דוחות כריתה

בסרגל הצד בחר/י:

> **📊 BI – דוחות כריתה**

במסך זה תמצא/י:

1. **רכיב העלאת קובץ**  
   - העלה/י את `merged_forest_reports_FINAL_dates_fixed.xlsx`.

2. **מסננים (פילטרים)**  
   - שנה.
   - יישוב.
   - סוג פעולה (כריתה / העתקה).
   - סיבה (בטיחות / בנייה / אחר).
   - אזור / מחוז ועוד.

3. **אוסף גרפים ו־TOP-10**  
   - TOP-10 יישובים בכמות כריתות  
     (ספירה על בסיס `__is_cut__ = True`).
   - TOP-10 מיני עץ שנכרתו.
   - פילוח לפי:
     - סיבה.
     - אזור.
     - טווחי תאריכים.

ניתן:

- לסנן לפי שנה/יישוב/סיבה.
- להוריד גרפים כ־PNG ולהכניס למצגות.
- להוריד קובצי CSV מסוננים להמשך ניתוח.

---

## 📝 ניתוח דוחות ערעורים – דף 📝 BI ערעורים

בסרגל הצד בחר/י:

> **📝 BI – דוחות ערעורים**

במסך זה תמצא/י:

1. **רכיב העלאת קובץ ערעורים**  
   - לדוגמה:  
     `forestry_and_trees_ararim2024- דוח ערערים.xlsx`

2. **מנגנון אוטומטי שמזהה:**
   - תאריך הערעור.
   - יישוב (מתוך כתובת/טקסט).
   - סיבת הערעור:
     - בנייה.
     - בטיחות.
     - מטרד.
     - אחר.
   - סטטוס ההחלטה:
     - התקבל.
     - התקבל חלקית.
     - נדחה.
     - לא נדון.
   - מספר עצים לשימור שניצלו.

3. **תוצרים ויזואליים:**
   - TOP-10 יישובים בכמות ערעורים.
   - יישובים מובילים בהצלחת ערעורים.
   - אחוזי הצלחה לפי סיבת ערעור.
   - רשימת ערעורים "גדולים" שהצילו הכי הרבה עצים.

---

## 🧭 מסלול עבודה מומלץ – סיכום

1. **איסוף קבצים**
   - ודא/י שכל ארבעת הקבצים קיימים:
     - דוחות כריתה מאוחדים ללא רשימות.
     - רשימת ערים.
     - רשימת עצים.
     - דוח ערעורים.

2. **הפרדת רשימות**
   - יצירת שלושת קבצי הכריתה:
     - דוחות כריתה ללא רשימות.
     - רשימת ערים לפי קודים.
     - רשימת עצים לפי קודים.

3. **🧩 Merge & Export**
   - הרצת המיזוג.
   - הפקת הקובץ `merged_forest_reports_FINAL_dates_fixed.xlsx`.

4. **📊 BI כריתה**
   - טעינת קובץ המיזוג.
   - ניתוח דוחות הכריתה.

5. **📝 BI ערעורים**
   - טעינת דוח הערעורים.
   - ניתוח ההחלטות והצלחות.

6. **ייצוא והגשה**
   - הורדת גרפים / CSV.
   - שילוב בדוחות, מצגות, עבודות אקדמיות וכו'.

---

## ⚙️ התקנה והרצה טכנית (Python + Streamlit)

### 1. שכפול הריפו

```bash
git clone https://github.com/24BlackMamba8/Forestry-Trees-2024.git
cd Forestry-Trees-2024
